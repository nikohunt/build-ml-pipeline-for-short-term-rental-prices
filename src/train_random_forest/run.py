...
def go(args):
    ...
    # Create a wandb.Artifact instance called "artifact"
    ...
    # Add the temp directory using .add_dir then log and wait so that temp dir
    # is not deleted prior to upload finishing
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # Note that we do not need to impute room_type because the type of the room
    # is mandatory on the websites, so missing values are not possible in production
    # (nor during training). That is not true for neighbourhood_group
    ordinal_categorical_preproc = OrdinalEncoder()

    # Non-ordinal categorical preprocessing pipeline:
